...
const getUser = async (req, res) => {
  ...
  try {
    const reqdata = data.find(
      (obj) => obj.name.toLowerCase() === username.toLowerCase()
    );
    const recusers = [data[0], data[1], data[2]];
    if (!reqdata) return res.status(200).json({rec:recusers});
    return res.status(200).json({ response: reqdata, rec: recusers });
  } catch (error) {
